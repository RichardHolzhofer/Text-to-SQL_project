...
class Config:
    ...
    def __init__(self, logger_name: str = "text-to-sql"):
        try:
            ...
            if hasattr(self, "logger"):
                self.logger.exception("Configuration initialization failed")
            raise error
    ...
    def load_ingestion_config(self) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.exception("Failed to load ingestion configuration")
            raise ConfigError(error) from error
    ...
    def get_smart_llm(self):
        ...
        try:
            ...
            self.logger.exception(f"Failed to initialize Smart LLM: {self.smart_model}")
            raise LLMException(error) from error
    ...
    def get_fast_llm(self):
        ...
        try:
            ...
            self.logger.exception(f"Failed to initialize Fast LLM: {self.fast_model}")
            raise LLMException(error) from error
    ...
    def get_llm(self, model_name: str):
        ...
        try:
            ...
            self.logger.exception(f"Failed to instantiate LLM for model: {model_name}")
            raise LLMException(error) from error
    ...
    def get_embedding_model(self):
        ...
        try:
            ...
            self.logger.exception(
                f"Failed to initialize OpenAI Embedding model: {self.embedding_model}"
            )
            raise EmbeddingCreationError(error) from error
    ...
    def reload_mcp_credentials(self) -> tuple[str | None, str | None, str | None]:
        ...
        try:
            ...
            self.logger.exception("Failed to reload MCP credentials from environment")
            raise ConfigError(error) from error
