...
def test_get_asgi_app_uses_sse_app(
    mcp_server,
):
    mcp_server.mcp.sse_app = Mock(return_value="sse")

    result = mcp_server.get_asgi_app()

    assert result == "sse"


def test_get_asgi_app_fallback_http_app(mcp_server):
    mcp_server.mcp.sse_app = Mock(side_effect=AttributeError())
    mcp_server.mcp.http_app = Mock(return_value="http-app")

    result = mcp_server.get_asgi_app()

    assert result == "http-app"

    mcp_server.mcp.sse_app.assert_called_once()
    mcp_server.mcp.http_app.assert_called_once()


def test_get_asgi_app_fallback_asgi(mcp_server):
    mcp_server.mcp.sse_app = Mock(side_effect=AttributeError())
    mcp_server.mcp.http_app = Mock(side_effect=AttributeError())
    mcp_server.mcp.asgi = Mock(return_value="asgi-app")

    result = mcp_server.get_asgi_app()

    assert result == "asgi-app"

    mcp_server.mcp.sse_app.assert_called_once()
    mcp_server.mcp.http_app.assert_called_once()
    mcp_server.mcp.asgi.assert_called_once()


def test_get_asgi_app_fallback_create_asgi_app(mcp_server):
    mcp_server.mcp.sse_app = Mock(side_effect=AttributeError())
    mcp_server.mcp.http_app = Mock(side_effect=AttributeError())
    mcp_server.mcp.asgi = Mock(side_effect=AttributeError())
    mcp_server.mcp.create_asgi_app = Mock(return_value="create-asgi-app")

    result = mcp_server.get_asgi_app()

    assert result == "create-asgi-app"

    mcp_server.mcp.sse_app.assert_called_once()
    mcp_server.mcp.http_app.assert_called_once()
    mcp_server.mcp.asgi.assert_called_once()
    mcp_server.mcp.create_asgi_app.assert_called_once()


def test_get_asgi_app_returns_mcp_instance(mcp_server):
    mcp_server.mcp.sse_app = Mock(side_effect=AttributeError())
    mcp_server.mcp.http_app = Mock(side_effect=AttributeError())
    mcp_server.mcp.asgi = Mock(side_effect=AttributeError())
    mcp_server.mcp.create_asgi_app = Mock(side_effect=AttributeError())

    result = mcp_server.get_asgi_app()

    assert result == mcp_server.mcp
