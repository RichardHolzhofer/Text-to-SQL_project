...
def test_config_uses_default_temperature_when_missing(minimal_required_env):
    with patch.dict(os.environ, {"LLM_TEMPERATURE": ""}, clear=False):
        config = _build_config(minimal_required_env)
    assert config.temperature == 0.0


def test_config_raises_for_invalid_temperature(minimal_required_env):
    with patch.dict(os.environ, {"LLM_TEMPERATURE": "not-a-number"}, clear=False):
        with pytest.raises(ConfigError):
            _build_config(minimal_required_env)


def test_config_raises_when_required_var_missing(minimal_required_env):
